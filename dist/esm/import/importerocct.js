...
export var ImporterOcct = /*#__PURE__*/function (_ImporterBase) {
  _inherits(ImporterOcct, _ImporterBase);

  var _super = _createSuper(ImporterOcct);
  ...
  _createClass(ImporterOcct, [{
    ...
    value: function ImportContent(fileContent, onFinish) {
      ...
      this.worker.addEventListener('error', function (ev) {
        _this2.SetError('Failed to load occt-import-js.');
    ...
    value: function ImportMesh(occtMesh, colorToMaterial) {
      ...
      if (occtMesh.color) {
        var color = RGBColorFromFloatComponents(occtMesh.color[0], occtMesh.color[1], occtMesh.color[2]);
        materialIndex = colorToMaterial.GetMaterialIndex(color.r, color.g, color.b, null);
      }

      var mesh = ConvertThreeGeometryToMesh(occtMesh, materialIndex);
      ...
      if (occtMesh.face_colors) {
        var _iterator3 = _createForOfIteratorHelper(occtMesh.face_colors),
            _step3;

        try {
          for (_iterator3.s(); !(_step3 = _iterator3.n()).done;) {
            var faceColorGroup = _step3.value;
            var faceColor = RGBColorFromFloatComponents(faceColorGroup.color[0], faceColorGroup.color[1], faceColorGroup.color[2]);
            var faceMaterialIndex = colorToMaterial.GetMaterialIndex(faceColor.r, faceColor.g, faceColor.b, null);

            for (var i = faceColorGroup.first; i <= faceColorGroup.last; i++) {
